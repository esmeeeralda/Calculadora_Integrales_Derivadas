...
// Instancias de clases para cálculos específicos
    ...
    private static boolean letra = false;// Variable de control
    
// Método para enrrutar (determinar la acción a realizar en base a la operación)
            ...
            for (int i = 0; i < Segmentos.size(); i++) {
                for (int j = 0; j < Segmentos.get(i).toString().length(); j++) {
                    if (Segmentos.get(i).toString().charAt(j) == '(') {
                        ...
                        String Cadena = ""; // Verificar el tipo de operación
                        for (ExpresionAlgebraica ccc1 : ccc) {
                            Cadena = Cadena
                                    + ccc1.getSimbolo()
                                    + ccc1.getCoeficiente()
...
//La clase Enrrutarparece ser una parte crucial del sistema que toma una lista de partes y delimitadores, 
//así como la operación ( op) y una diferencial.

//Se crean "instExponencial", "derivadaPotencia", etc.)

//Se utiliza una variable de control "letra" que se ajusta en diferentes partes del código

//El metodo `Enrrut" Enrrutador es  el corazón de la lógica

//Dependiendo de la operación (op), se ejecutan diferentes (op) 
//No Dentro de cada bloque de los resultados se almacenan en la variable resultado.
